...
class add_del
{
 ...
 //   {
	  //get balance
	  $addedbalance=$mybalance-$amount;
	  //write to db
	  $tbl_name2="balance";
      $sqlv="UPDATE `$tbl_name2` SET `ab` = '$addedbalance'  WHERE id='$fromuserid'";
  ...
  //if ($process=='SUB')
	 ...
	 $subtractbalance=$got_balance_sub+$amount;
	 //write to db
	$tbl_name2="balance";
     $sqlv="UPDATE `$tbl_name2` SET `ab` = '$subtractbalance'  WHERE id='$touserid'";
     $resultv=mysql_query($sqlv);
     if ($resultv)
      { 
	 $msg_re=1;
